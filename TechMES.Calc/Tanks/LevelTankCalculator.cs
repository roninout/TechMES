...
public static class LevelTankCalculator
{
    ...
    /// calculateAbove100 - разрешает продолжать расчёт Volume выше 100 % внутри upper dead area.
    ///
    /// Level при этом НИКОГДА не ограничивается сверху 100 %.
    /// Ограничивается только физическая высота, передаваемая в геометрический расчёт Volume.
    /// </summary>
    public static LevelTankResult Calculate(double levelRaw, double densityHmi, double totalLengthMm, double lowerDeadAreaMm, double upperDeadAreaMm, bool calculateAbove100, Func<double, double> calculateVolume)
    {
        ...
        var measurementAreaMm = totalLengthMm - lowerDeadAreaMm - upperDeadAreaMm;
         ...
         // Level.R приходит в реальных процентах. 58.6 -> 58.6 %
         // Снизу сохраняем старую защиту: отрицательный Level не используем.
         // Сверху НЕ ограничиваем: 105 %, 110 % и т.д. остаются допустимыми.
        var levelMm = Math.Max(0.0, measurementAreaMm * levelRaw / 100.0);

         // Физическая высота жидкости от самого нижнего края Tank.
         // При 0 %: liquidHeight = lowerDeadArea.
         // При 100 %: liquidHeight = lowerDeadArea + measurementArea.
        var liquidHeightMm = lowerDeadAreaMm + levelMm;

         // Если расчёт выше 100 % запрещён, Volume останавливается на верхней границе рабочей зоны.
        ...
        if (!calculateAbove100)
         ...
         // Даже при calculateAbove100=true нельзя считать виртуальный объём выше физической геометрии Tank.
