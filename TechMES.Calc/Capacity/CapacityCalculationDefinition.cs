...
/// - Pressure уже присутствует в контракте и может быть настроено,
///   но пока имеет DefaultValue и не влияет на текущую legacy-формулу.
///
/// Это позволяет не создавать ложную зависимость расчёта от Pressure сегодня,
/// но при этом оставить нормальное место для будущих формул Capacity,
/// которые действительно будут зависеть от давления.
///
/// Как и Density, этот Definition не ограничен двумя физическими параметрами.
/// Новые именованные параметры добавляются в PropertyParameterDefinitions.
/// </summary>
public sealed class CapacityCalculationDefinition : MixtureCalculationDefinitionBase
{
    ...
    [
        ...
        /*
         * Pressure уже является частью Definition, потому что:
         * - оно присутствовало в старой Capacity configuration;
         * - будущая физическая модель может его использовать;
         * - специализированный WEB UI сможет сохранить Pressure binding уже сейчас.
         *
         * Но текущая перенесённая формула Pressure не использует.
         * Поэтому отсутствие binding не должно останавливать Capacity Job.
         *
         * В дальнейшем, если новая версия формулы действительно начнёт
         * зависеть от Pressure, мы сделаем параметр IsRequired=true
         * и увеличим DefinitionVersion.
         */
        new CalculationParameterDefinition(
            Key: PressureKey,
         ...
         * Старый Capacity:
         *
         * Mix.GetCapacity() уже возвращал J/(kg·K).
         * DELTA_C из OPC умножался на 10,
         * а при расчёте затем использовался DeltaC * 0.1.
         *
         * Эти операции взаимно уничтожаются.
         *
         * Поэтому новый Capacity correction можно напрямую считать
         * инженерным значением в J/(kg·K).
         */
        new CalculationParameterDefinition(
            Key: CorrectionKey,
    ...
    ///
    /// Pressure читается из ParameterSet и присутствует в Trace,
    /// но текущий перенесённый MixturePropertyCalculator использует
    /// для Capacity только Temperature.
    ///
    /// Это сделано намеренно, а не является потерянным параметром.
    /// </summary>
    protected override CalculationResult CalculateCore(CalculationParameterSet parameters, bool includeTrace)
    {
        ...
        if (!double.IsFinite(capacityJPerKgK) || capacityJPerKgK <= 0d)
        {
            return CalculationResult.Failure(
                ...
                "Calculated specific heat capacity after correction must be a finite value greater than zero.");
        }
