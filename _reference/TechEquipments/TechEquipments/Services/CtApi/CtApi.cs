    ...
    public class CtApi : IDisposable
    {
        ...
        public string Cicode(string cmd, uint win = 0)
        {
            ...
            if (result == 0)
            {
                ...
                //Logger.Error($"Citect.CtApi > Cicode, cmd={cmd}, win={win}, error={error.Message}");
                Logger.Error($"Citect.CtApi > Cicode, cmd={cmd}, win={win}, error=error");
                //throw error;
        ...
        public void Close()
        {
            if (_ctapi != IntPtr.Zero)
            {
                Logger.Info($"Citect.CtApi > Close");

                var result = CtClose(_ctapi);
                if (result == false)
                {
                    var error = new Win32Exception(Marshal.GetLastWin32Error());
                    Logger.Error($"Citect.CtApi > Close, error={error.Message}");
                    throw error;
                }
        ...
        private string GetProperty(IntPtr hfindptr, string propertyName, bool isLogger = false)
        {
            if (isLogger)
                Logger.Trace($"Citect.CtApi > GetProperty, propertyName={propertyName}");
            ...
            {
                var error = new Win32Exception(Marshal.GetLastWin32Error());
                Logger.Error($"Citect.CtApi > GetProperty, propertyName={propertyName}, error={error.Message}");
                return null;
            }
            else
            {
                if (isLogger)
                    Logger.Trace($"Citect.CtApi > GetProperty, propertyName={propertyName}, propertyValue={pData}");
        ...
        {
            Logger.Debug($"Citect.CtApi > ListAdd, list={list}, tag={tag}");

            var result = CtListAdd(list, tag);
            if (result == IntPtr.Zero)
            {
                var error = new Win32Exception(Marshal.GetLastWin32Error());
                Logger.Error($"Citect.CtApi > ListAdd list={list}, tag={tag}, error={error.Message}");
                throw error;
            }
            else
            {
                Logger.Debug($"Citect.CtApi > ListAdd, list={list}, tag={tag}, result={result}");
                return result;
            }
        }
        ...
        {
            Logger.Debug($"Citect.CtApi > ListAddEx, list={list}, tag={tag}, raw={raw}, pollPeriodMS={pollPeriodMS}, deadband={deadband}");

            var result = CtListAdd(list, tag);
            if (result == IntPtr.Zero)
            {
                var error = new Win32Exception(Marshal.GetLastWin32Error());
                Logger.Error($"Citect.CtApi > ListAddEx list={list}, tag={tag}, raw={raw}, pollPeriodMS={pollPeriodMS}, deadband={deadband}, error={error.Message}");
                throw error;
            }
            else
            {
                Logger.Debug($"Citect.CtApi > ListAddEx, list={list}, tag={tag}, raw={raw}, pollPeriodMS={pollPeriodMS}, deadband={deadband}, result={result}");
                return result;
            }
        }
        ...
        {
            Logger.Debug($"Citect.CtApi > ListData, tag={tag}, mode={mode}");
            ...
            {
                var error = new Win32Exception(Marshal.GetLastWin32Error());
                Logger.Error($"Citect.CtApi > ListData, tag={tag}, mode={mode}, error={error.Message}");
                throw error;
            }
            else
            {
                Logger.Debug($"Citect.CtApi > ListData, tag={tag}, mode={mode}, result={result}, value={value}");
                return value.ToString();
            }
        }
        ...
        {
            Logger.Debug($"Citect.CtApi > ListDelete, tag={tag}");

            var result = CtListDelete(tag);
            if (result == false)
            {
                var error = new Win32Exception(Marshal.GetLastWin32Error());
                Logger.Error($"Citect.CtApi > ListDelete tag={tag}, error={error.Message}");
                throw error;
            }
            else
            {
                Logger.Debug($"Citect.CtApi > ListDelete, tag={tag}, result={result}");
            }
        }
        ...
        {
            Logger.Debug($"Citect.CtApi > ListEvent, list={list}");
            var result = CtListEvent(list, mode);
            Logger.Debug($"Citect.CtApi > ListEvent, list={list}, result={result}");
        ...
        {
            Logger.Debug($"Citect.CtApi > ListFree, list={list}");

            var result = CtListFree(list);
            if (result == false)
            {
                var error = new Win32Exception(Marshal.GetLastWin32Error());
                Logger.Error($"Citect.CtApi > ListFree list={list}, error={error.Message}");
                throw error;
            }
            else
            {
                Logger.Debug($"Citect.CtApi > ListFree, list={list}, result={result}");
            }
        }
        ...
        {
            Logger.Debug($"Citect.CtApi > ListItem, tag={tag}, item={item}, mode={mode}");
            ...
            {
                var error = new Win32Exception(Marshal.GetLastWin32Error());
                Logger.Error($"Citect.CtApi > ListItem, tag={tag}, item={item}, mode={mode}, error={error.Message}");
                throw error;
            }
            else
            {
                Logger.Debug($"Citect.CtApi > ListItem, tag={tag}, item={item}, mode={mode}, result={result}, value={value}");
                return value.ToString();
            }
        }
        ...
        {
            Logger.Debug($"Citect.CtApi > ListNew, mode={mode}");

            var result = CtListNew(_ctapi, mode);
            if (result == IntPtr.Zero)
            {
                var error = new Win32Exception(Marshal.GetLastWin32Error());
                Logger.Error($"Citect.CtApi > ListNew, mode={mode}, error={error.Message}");
                throw error;
            }
            else
            {
                Logger.Debug($"Citect.CtApi > ListNew, mode={mode}, result={result}");
                return result;
            }
        }
        ...
        {
            Logger.Debug($"Citect.CtApi > ListRead, list={list}, ctOverlapped={ctOverlapped}");

            var result = CtListRead(list, ctOverlapped);
            if (result == false)
            {
                var error = new Win32Exception(Marshal.GetLastWin32Error());
                Logger.Error($"Citect.CtApi > ListRead, list={list}, ctOverlapped={ctOverlapped}, error={error.Message}");
                throw error;
            }
            else
            {
                Logger.Debug($"Citect.CtApi > ListRead, list={list}, ctOverlapped={ctOverlapped}, result={result}");
            }
        }
        ...
        {
            Logger.Debug($"Citect.CtApi > ListWrite, tag={tag}, value={value}");

            var result = CtListWrite(tag, value, ctOverlapped);
            if (result == false)
            {
                var error = new Win32Exception(Marshal.GetLastWin32Error());
                Logger.Error($"Citect.CtApi > ListWrite tag={tag}, value={value}, error={error.Message}");
                throw error;
            }
            else
            {
                Logger.Debug($"Citect.CtApi > ListWrite, tag={tag}, value={value}, result={result}");
                return;
            }
        }
        ...
        public void Open(string computer = null, string user = null, string password = null, CtOpen? mode = null)
        {
            ...
            Logger.Info($"Citect.CtApi > Open, computer={computer}, user={user}");
            _ctapi = CtOpen(computer, user, password, (uint)(mode ?? 0));

            if (_ctapi == IntPtr.Zero)
            {
                var error = new Win32Exception(Marshal.GetLastWin32Error());
                Logger.Error($"Citect.CtApi > Open, computer={computer}, user={user}, error={error.Message}");
                throw error;
            }
        }
        ...
        {
            Logger.Info($"Citect.CtApi > SetCtApiDirectory, path={path}");
        ...
        private void SetManagedBinDirectory(string path)
        {
            try
            {
                Logger.Info($"Citect.CtApi > SetManagedBinDirectory, path={path}");
                var result = CtSetManagedBinDirectory(path);
                Logger.Info($"Citect.CtApi > SetManagedBinDirectory, path={path}, result={result}");
            }
            catch (Exception e)
            {
                Logger.Warning($"Citect.CtApi > SetManagedBinDirectory, error={e.Message}");
                Logger.Trace($"Citect.CtApi > SetManagedBinDirectory, error={e}");
            }
        }
        ...
        /// <param name="tag">The tag name or tag name and element name, separated by a dot. If the element name is not specified, it will be resolved at runtime as for an unqualified tag reference. You may use the array syntax [] to select an element of an array.</param>
        /// <exception cref="Win32Exception"></exception>
        public string TagRead(string tag, bool isLogger = false)
        {
            var value = new StringBuilder(100);
            var result = CtCicode(_ctapi, $"TagRead({tag})", 0, 0, value, value.Capacity, IntPtr.Zero);

            if (isLogger)
            {
                if (result == 0)
                    Logger.Error($"Citect.CtApi > Cicode TagRead, tag = {tag}, value = {value}, error = read with error");
                else
                    Logger.Debug($"Citect.CtApi > Cicode TagRead, tag = {tag}, value = {value}");
            }

            return value.ToString();
        }

        public string TagReadSync(string tag)
        {
            string valResult = "error";

            var value = new StringBuilder(100);
            var result = CtTagRead(_ctapi, tag, value, value.Capacity);
            if (result != false)
                valResult = value.ToString();

            Logger.Debug($"Citect.CtApi > TagRead, tag = {tag}, value = {valResult}");
            return valResult;
        }

        public string TagReadWithDebug(string tag)
        {
            Logger.Debug($"Citect.CtApi > TagRead, tag={tag}");
            ...
            {
                var error = new Win32Exception(Marshal.GetLastWin32Error());
                Logger.Error($"Citect.CtApi > TagRead, tag={tag}, error={error.Message}");
                throw error;
            }
            else
            {
                Logger.Debug($"Citect.CtApi > TagRead, tag={tag}, value={value}");
                return value.ToString();
            }
        }
        ...
        /// <exception cref="Win32Exception"></exception>
        public Task TagWriteAsync(string tag, string value, bool isLogger = false)
        {
            return Task.Run(() => TagWrite(tag, value, isLogger));
        }
        ...
        /// <param name="value">The value to write to the tag as a string.</param>
        /// <exception cref="Win32Exception"></exception>
        public void TagWrite(string tag, string value, bool isLogger = false)
        {
            var strBuid = new StringBuilder(100);
            var result = CtCicode(_ctapi, $"TagWrite({tag}, {value})", 0, 0, strBuid, strBuid.Capacity, IntPtr.Zero);
            ...
            if (isLogger)
            {
                ...
                if (result == 0)
                //if (result == false)
                    Logger.Error($"Citect.CtApi > Cicode TagWrite, tag = {tag}, value = {value}, error = write with error");
                else
                    Logger.Info($"Citect.CtApi > Cicode TagWrite, tag = {tag}, value = {value}");
            }
        }


        public void TagWriteSync(string tag, string value)
        {
            var result = CtTagWrite(_ctapi, tag, value);
            if (result == false)
            {
                Logger.Error($"Citect.CtApi > TagWrite, tag = {tag}, value = {value}, error = write with error");
            }
            else
                Logger.Debug($"Citect.CtApi > TagWrite, tag = {tag}, value = {value}");
        }

        public void TagWriteWithDebug(string tag, string value)
        {
            Logger.Debug($"Citect.CtApi > TagWrite, tag={tag}, value={value}");
            ...
            {
                var error = new Win32Exception(Marshal.GetLastWin32Error());
                Logger.Debug($"Citect.CtApi > TagWrite, tag={tag}, value={value}, error={error.Message}");
                throw error;
            }
        }
