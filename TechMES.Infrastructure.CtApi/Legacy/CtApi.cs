    ...
    public class CtApi : IDisposable
    {
        ...
        public string Cicode(string cmd, uint win = 0)
        {
            ...
            if (result == 0)
            {
                ...
                _logger?.LogError($"Citect.CtApi > Cicode, cmd={cmd}, win={win}, error=error");
                //throw error;
        ...
        public string TagRead(string tag, bool isLogger = false)
        {
            ...
            {
                if (result == 0)
                    _logger?.LogError($"Citect.CtApi > Cicode TagRead, tag = {tag}, value = {value}, error = read with error");
                else
                    _logger?.LogDebug($"Citect.CtApi > Cicode TagRead, tag = {tag}, value = {value}");
            }
        ...
        public string TagReadSync(string tag)
        {
            ...
            _logger?.LogDebug($"Citect.CtApi > TagRead, tag = {tag}, value = {valResult}");
            return valResult;
        }
        ...
        public void TagWrite(string tag, string value, bool isLogger = false)
        {
            ...
            if (isLogger)
            {
                ...
                if (result == 0)
                //if (result == false)
                    _logger?.LogError($"Citect.CtApi > Cicode TagWrite, tag = {tag}, value = {value}, error = write with error");
                else
                    _logger?.LogInformation($"Citect.CtApi > Cicode TagWrite, tag = {tag}, value = {value}");
            }
        }
        ...
        public void TagWriteSync(string tag, string value)
        {
            ...
            if (result == false)
            {
                _logger?.LogError($"Citect.CtApi > TagWrite, tag = {tag}, value = {value}, error = write with error");
            }
            else
